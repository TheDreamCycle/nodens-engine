...
OpenGLShader::OpenGLShader(const std::string& vertexSrc,
  ...
  if (isCompiled == GL_FALSE) {
    ...
    ND_CORE_ERROR("{0}", infoLog.data());
    ND_CORE_ASSERT(false, "Vertex shader compilation failed!");
    return;
  }
  ...
  if (isCompiled == GL_FALSE) {
    ...
    // Use the infoLog as you see fit.
    ND_CORE_ERROR("{0}", infoLog.data());
    ND_CORE_ASSERT(false, "Fragment shader compilation failed!");
  ...
  if (isLinked == GL_FALSE) {
    ...
    // Use the infoLog as you see fit.
    ND_CORE_ERROR("{0}", infoLog.data());
    ND_CORE_ASSERT(false, "OpenGLShader link failed!");
    return;
  }
...
void OpenGLShader::Use() const { glUseProgram(m_RendererID); }
