...
namespace Nodens {
	...
	void Renderer::SubmitIndexedLines(const Ref<Shader>& shader, const Ref<VertexArray>& vertexArray, const glm::mat4 transform, const glm::vec4 color)
	{
		shader->Use();
		std::dynamic_pointer_cast<OpenGLShader>(shader)->UploadUniformMat4("uViewProjection", m_SceneData->ViewProjectionMatrix);
		std::dynamic_pointer_cast<OpenGLShader>(shader)->UploadUniformMat4("uModelMatrix", transform);
	...
	void Renderer::SubmitIndexedPoints(const Ref<Shader>& shader, const Ref<VertexArray>& vertexArray, const glm::mat4 transform, const glm::vec4 color)
	{
		shader->Use();
		std::dynamic_pointer_cast<OpenGLShader>(shader)->UploadUniformMat4("uViewProjection", m_SceneData->ViewProjectionMatrix);
		std::dynamic_pointer_cast<OpenGLShader>(shader)->UploadUniformMat4("uModelMatrix", transform);
