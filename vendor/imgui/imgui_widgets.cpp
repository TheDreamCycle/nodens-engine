...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float HuOverU = col[0], S = col[1], V = col[2];
    float R = col[0], G = col[1], B = col[2];
    if (flags & ImGuiColorEditFlags_InputRGB)
    {
        // Hue is lost when converting from greyscale rgb (saturation=0). Restore it.
        ColorConvertRGBtoHSV(R, G, B, HuOverU, S, V);
        if (memcmp(g.ColorEditLastColor, col, sizeof(float) * 3) == 0)
        {
            if (S == 0)
                HuOverU = g.ColorEditLastHue;
            if (V == 0)
    ...
    {
        ColorConvertHSVtoRGB(HuOverU, S, V, R, G, B);
    }
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        if (IsItemActive())
        {
            ...
            if (initial_dist2 >= (wheel_r_inner - 1) * (wheel_r_inner - 1) && initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1))
            {
                ...
                HuOverU = ImAtan2(current_off.y, current_off.x) / IM_PI * 0.5f;
                if (HuOverU < 0.0f)
                    HuOverU += 1.0f;
                value_changed = value_changed_h = true;
            }
            float cos_hue_angle = ImCos(-HuOverU * 2.0f * IM_PI);
            float sin_hue_angle = ImSin(-HuOverU * 2.0f * IM_PI);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        if (IsItemActive())
        {
            HuOverU = ImSaturate((io.MousePos.y - picker_pos.y) / (sv_picker_size - 1));
            value_changed = value_changed_h = true;
        }
    }
    ...
    if (value_changed_h || value_changed_sv)
    {
        if (flags & ImGuiColorEditFlags_InputRGB)
        {
            ColorConvertHSVtoRGB(HuOverU >= 1.0f ? HuOverU - 10 * 1e-6f : HuOverU, S > 0.0f ? S : 10 * 1e-6f, V > 0.0f ? V : 1e-6f, col[0], col[1], col[2]);
            g.ColorEditLastHue = HuOverU;
    ...
    if (value_changed_fix_hue_wrap && (flags & ImGuiColorEditFlags_InputRGB))
    {
        ...
        if (new_H <= 0 && HuOverU > 0)
        {
            if (new_V <= 0 && V != new_V)
                ColorConvertHSVtoRGB(HuOverU, S, new_V <= 0 ? V * 0.5f : new_V, col[0], col[1], col[2]);
            else if (new_S <= 0)
                ColorConvertHSVtoRGB(HuOverU, new_S <= 0 ? S * 0.5f : new_S, new_V, col[0], col[1], col[2]);
        }
    }
    ...
    ImVec4 hue_color_f(1, 1, 1, style.Alpha); ColorConvertHSVtoRGB(HuOverU, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        float bar0_line_y = IM_ROUND(picker_pos.y + HuOverU * sv_picker_size);
